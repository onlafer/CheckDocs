...
btns_installing_kassis = create_buttons_inline(
    "✅ Все получилось", '➡️ 2 вариант (установка "Техэксперт")'
)
...
btns_end_link_verification = create_buttons_inline(
    "🔄 Начать проверку ссылок заново", "📞 Обратная связь", one_time_keyboard=True
)
...
kb_step_two = create_menus(
    ...
    links_with_irrelevant_information="Ссылки с неактульной информацией",
...
async def process_there_is_no_access(request: Message | CallbackQuery):
    ...
    await message.answer(
        ...
        "4. Ссылки на доступ из московских вузов - https://t.me/te_edu/295",
    )
    await message.answer(
        "Автоматическая проверка ссылок на нормативную документацию осуществляется с помощью утилиты <b>kAssist</b> (интеграция)."
    )
...
@user_private_router.message(
    F.text.lower().endswith("у меня не установлен <b>kassist</b> (не знаю)")
)
...
@user_private_router.message(
    F.text.lower().endswith('2 вариант (установка "техэксперт")')
)
...
async def process_from_another_university(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "Пример: http://172.16.3.18:8080/docs/"
    )
    await message.answer(
        "Хотите посмотреть инструкцию по установке?",
        reply_markup=btns_installation_instructions,
...
async def process_installation_instructions(message: Message):
    ...
    await message.answer(
        ...
        "Подробная видеоинструкция поустановке: https://www.youtube(СКРИН)"
    )
    await message.answer(
        "Если установка прошла успешно, вы увидите на своем экране:(СКРИН)"
    )
    await message.answer(
...
async def process_user_errors(request: Message | CallbackQuery):
    message: Message = getting_the_message_object(request)
    await message.answer(
        "1. Интеграция может не подклключиться, если были запущены приложения во время установки.\n\n"
    ...
    await message.answer("Если установка прошла успешно, вы увидите на своем экране:")
...
async def process_step_by_step_verification(message: Message):
    ...
    await message.answer(
        ...
        "а) синий цвет — для действующих документов;\n\n"
        "б) тёмно-красный — для недействующих документов;\n\n"
        "в) оранжевый — для документов с особым статусом.\n\n"
...
async def process_placed_links(request: CallbackQuery):
    message: Message = getting_the_message_object(request)
    await message.answer(
        "Автоматически расставленные гиперссылки (данные фрагменты текста имеют только один подходящий документ) (ДОБАВИТЬ КАРТИНКУ)",
...
async def process_several_suitable_document(request: CallbackQuery):
    message: Message = getting_the_message_object(request)
    await message.answer(
        "Фрагменты, имеющие несколько подходящих документов (нужный документ можно выбрать вручную из списка) (ДОБАВИТЬ КАРТИНКУ)",
        reply_markup=btns_for_links,
    )
...
async def process_unsuitable_documents(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "Так как иногда kAssist не распознает атрибуты документа, сформулированные некорректно. (ДОБАВИТЬ КАРТИНКУ)",
        reply_markup=btns_for_links,
    )
...
async def process_step_two(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "По итогам анализа ссылки будут разделены на 4 группы в окне проверки (ДОБАВИТЬ КАРТИНКУ)",
        reply_markup=ReplyKeyboardRemove(),
    )
    await message.answer("Ссылки разделены на 4 группы:", reply_markup=kb_step_two)
...
async def process_links_to_current_documents(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "Чтобы проверить актуальность редакции нужно перейти в текст документа и воспользоваться <b>Cервисом документ во времени</b>.",
...
async def process_links_with_irrelevant_information(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "Дальнейшая работа см. пункты 1-3 СКРИНШОТ",
        reply_markup=btns_another_links_for_second_step,
    )
...
async def process_independent_work_with_links(request: CallbackQuery):
    ...
    await message.answer(
        ...
        "1. Опции для самостоятельной работы со ссылками:\n"
        "а) Просмотр списка установленных гиперссылок;\n\n"
        "б) Открытие документов в системе или удаление гиперссылки;\n\n"
        "в) Выделение нужных гиперссылок цветом;\n\n"
        "г) Ручной поиск нужного документа в системе, на который будет направлена ссылка. СКРИНШОТ",
        reply_markup=btns_another_links_for_second_step,
    )
...
# обратная связь
@user_private_router.message(
    or_f(Command("about"), F.text.lower().endswith("обратная связь"))
)
